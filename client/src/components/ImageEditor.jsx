...
import { Download, RotateCw, FlipHorizontal, FlipVertical, Image as ImageIcon, Maximize, FileDigit } from 'lucide-react';
...
const ImageEditor = ({ file }) => {
  ...
  const onCropComplete = useCallback((croppedArea, croppedAreaPixels) => {
    ...
    // Update resize dimensions based on the crop selection if not manually overridden
    // Logic: If user is cropping, the natural output size changes.
    // However, to keep it simple, we'll let resize be an explicit override.
  }, []);
  ...
  const handleResizeChange = (e) => {
    ...
    // Calculate aspect ratio from original or current crop? 
    // Usually resize applies to the result. Let's use the current crop aspect if available, or original.
  ...
  const handleDownload = async () => {
    ...
    try {
      // 1. Get the cropped/transformed image first
      const croppedImageBlob = await getCroppedImg(
        ...
        outputFormat,
        quality,
        resizeDim // Pass resize dimensions to utility
      );
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-8 scrollbar-thin">
            ...
            <section>
                ...
                <div className="grid grid-cols-3 gap-2">
                    ...
                    className={`btn-tool ${flip.horizontal ? 'active' : ''}`}
                    ...
                    className={`btn-tool ${flip.vertical ? 'active' : ''}`}
                    title="Flip Vertical"
                    >
            ...
            <section>
                ...
                <div className="grid grid-cols-4 gap-2">
                    ...
                    <button
                        ...
                        className={`text-xs py-2 rounded border transition-colors ${aspect === opt.val ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                    >
                        {opt.label}
                    </button>
                    ))}
                ...
                <div className="flex items-center gap-2 mb-2">
                    <div className="relative w-full">
                        <input 
                            ...
                            className="w-full p-2 pl-8 border border-gray-200 rounded text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                            placeholder="Width"
                        />
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                         ...
                         <select 
                            ...
                            className="bg-white border border-gray-200 text-sm rounded p-1 focus:outline-none focus:border-blue-500"
                        >
